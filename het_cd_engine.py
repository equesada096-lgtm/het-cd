...
def patron_activo_para_calculo(patron_row: Any) -> bool:
    ...
    Regla general del modelo:
    - Solo compiten los patrones activos.
    - El grupo/subgrupo no interviene para elegir patrón; solo sirve para control de rango legal.
    - Si un patrón está bloqueado/inactivo, queda excluido antes de calcular el Top-K.

    Compatibilidad de datos:
    - Si no existe ninguna columna de control, el patrón se considera activo.
    - Si existe activo_calculo, solo son activos los valores afirmativos.
    - También se reconocen, de forma defensiva, columnas frecuentes como bloqueado, inactivo o estado.
    """
    afirmativos = {"SÍ", "SI", "S", "TRUE", "1", "YES", "Y", "ACTIVO", "VALIDADO"}
    negativos = {"NO", "N", "FALSE", "0", "INACTIVO", "BLOQUEADO", "BLOQUEADA", "EXCLUIDO", "EXCLUIDA", "DESACTIVADO", "DESACTIVADA"}

    # Columnas de bloqueo explícito: si existen y son afirmativas, excluyen el patrón.
    for col in ("bloqueado", "bloqueada", "inactivo", "excluir_calculo", "excluido_calculo"):
        valor_bloq = row_get(patron_row, col, None)
        if valor_bloq is not None and str(valor_bloq).strip() != "":
            texto_bloq = str(valor_bloq).strip().upper()
            if texto_bloq in afirmativos:
                return False

    # Columna de estado textual, si existe.
    estado = row_get(patron_row, "estado", None)
    if estado is not None and str(estado).strip() != "":
        texto_estado = str(estado).strip().upper()
        if texto_estado in negativos:
            return False
        if texto_estado in afirmativos:
            return True

    # Columna principal del modelo.
