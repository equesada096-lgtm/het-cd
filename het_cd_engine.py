...
def classify_het_cd(
    ...
    cd_ajustado = ajustar_cd_a_rango(cd_recomendado, validacion)
    diferencial_cd = None
    ...
    return {
        ...
        "resultado_cd": {
            ...
            "nota_rango": "El CD final se ha limitado por el rango legal del grupo/subgrupo." if cd_recomendado != cd_ajustado else "El CD K1 orientativo se mantiene dentro del rango legal.",
        },
...
def generar_texto_recomendacion(result: Dict[str, Any]) -> Dict[str, str]:
    ...
    if estado != RANGO_OK:
        ...
        titulo = "Mantener CD"
        texto = (
            f"El CD vigente ({cd_vig}) resulta coherente con el patrón técnico más próximo y se mantiene dentro del rango legal aplicable. "
        ...
        texto = (
            f"El puesto presenta mayor proximidad técnica con patrones de referencia de CD {cd_k1}. Tras aplicar el rango legal, "
            f"el CD final de referencia queda en {cd_final}, frente al CD vigente {cd_vig}. Procede analizar la revisión al alza, "
    ...
    if str(cd_k1) != str(cd_final):
        texto += (
            f" El patrón K1 apunta a CD {cd_k1}, pero el resultado final se ajusta a CD {cd_final} por aplicación del rango legal "
            f"del grupo/subgrupo ({val.get('cd_min')} - {val.get('cd_max')})."
        )
