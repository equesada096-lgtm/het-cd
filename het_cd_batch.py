...
<div class="box"><p><strong>Estado rango:</strong> {escape(str(val.get('estado','')))}. {escape(str(val.get('mensaje','')))}</p><p><strong>CD K1 orientativo:</strong> {escape(str(res.get('cd_tecnico_recomendado','')))} · <strong>CD recomendado:</strong> {escape(str(res.get('cd_tecnico_ajustado','')))} · <strong>Diferencial:</strong> {escape(str(res.get('diferencial_cd','')))}</p><p><strong>Recomendación:</strong> {escape(str(rec.get('titulo','')))}</p><p>{escape(str(rec.get('texto','')))}</p></div>
<h2>3. Similitud</h2>
...
<p>El informe resume la evaluación sistemática de los puestos tipo cargados en puestos_vector. La recomendación se basa en el patrón K1 activo más próximo. El rango legal del grupo/subgrupo se informa como control de validez, sin recortar automáticamente la recomendación.</p>
