...
def build_global_report(summary: pd.DataFrame, agregado: Dict[str, Any]) -> str:
    ...
    if not summary.empty and "diferencial_cd" in summary.columns:
        def _sentido(v):
            try:
                n = int(float(v))
            except Exception:
            ...
            if n < 0:
                return "A_LA_BAJA"
            return "MANTENER"
        summary["sentido_revision"] = summary["diferencial_cd"].apply(_sentido)
    ...
    prioritarios = summary[summary["resultado_preliminar"].isin(["ANALIZAR_AL_ALZA", "ANALIZAR_A_LA_BAJA", "REVISION_AGRUPADA", "INCIDENCIA_NORMATIVA"])] if "resultado_preliminar" in summary.columns else pd.DataFrame()
    sim_cols = [c for c in ["id_het", "denominacion_normalizada", "grupo_subgrupo", "cd_vigente", "patron_k1", "nombre_patron_k1", "cd_k1_orientativo", "similitud_total_k1", "similitud_funcional_k1", "similitud_factores_cd_k1", "sentido_revision", "resultado_preliminar"] if c in summary.columns]
